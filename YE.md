...
- Key bindings: Shift+Tab cycles mode, Ctrl+O toggles verbose output, Ctrl+G composes the buffer in `$VISUAL`/`$EDITOR`, Ctrl+R searches cross-session prompt history. `!cmd` runs a one-off shell command, `@` opens the file-mention picker, `/` opens the command picker (↑↓ move, Enter runs the highlighted command, Tab completes it into the buffer, Esc dismisses).
