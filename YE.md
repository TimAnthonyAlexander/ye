...
Tools: 19 tools registered in `src/tools/registry.ts` — Read, Edit, Write, Bash, BashOutput, KillShell, Grep, Glob, TodoWrite, ExitPlanMode, EnterPlanMode, AskUserQuestion, Task, TaskOutput, KillAgent, WebFetch, WebSearch, Skill, SaveMemory. Read/Glob/Grep/AskUserQuestion/Skill/WebFetch/WebSearch/BashOutput/TaskOutput are read-only (auto-allow in NORMAL); the rest prompt. `Task` spawns subagents (in-process, isolated state). `ExitPlanMode` / `EnterPlanMode` trigger mode-flip permission prompts. Note the PLAN allowlist (`src/permissions/modes.ts`) is narrower than "read-only": BashOutput/TaskOutput are not on it and are stripped from the pool in PLAN.

Async work: `Task` runs in the background by default, `Bash` on `run_in_background: true`. Each manager (`src/subagents/background.ts`, `src/tools/bash/background.ts`) holds tasks until a turn drains them into history as `<system-reminder>`s (`pipeline/turn.ts`). `pipeline/backgroundWakeup.ts` is the shared waiter: it races both managers so neither kind blocks the other's wakeup, and both the Ink loop (`app.tsx`) and headless (`pipeline/headless.ts`) pump turns off it — without that pump headless would exit before background results arrived. Background tasks default to the 900s timeout ceiling, not the 120s foreground default.
