...
Ye is a local Claude Code clone. TypeScript on Bun, terminal UI in React/Ink, multi-provider (OpenRouter, Anthropic-direct, OpenAI, DeepSeek native, and Ollama for local models), all state on disk under `~/.ye/`.
...
3. **Providers** (`providers/`) — abstract `Provider` interface; OpenRouter v1, Anthropic + OpenAI Phase 3, DeepSeek native (with reasoning round-trip), Ollama (local) — all shipped. See `PROVIDERS.md`.
...
- Slash commands (`/mode`, `/clear`, `/help`, `/init`, `/provider`, `/model`, `/routing`, `/exit`)
...
- DeepSeek native (with reasoning_content round-trip on tool-call sub-loops) — **shipped**. The canonical path for V4 Pro reasoning; OpenRouter strips reasoning fields on every V4 Pro upstream except DeepInfra (66k context). See `REASONING_STORING.md` for the empirical matrix.
- Reasoning preservation across turns: `Message.reasoning_details` canonical form persisted to JSONL transcript, replayed via `/resume` and `/rewind`, stripped on `/model` / `/provider` switches — **shipped**
