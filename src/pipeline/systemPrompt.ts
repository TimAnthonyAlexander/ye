...
**Background tasks:** set \`run_in_background: true\` for long commands (builds, test suites, installs). The command runs in the background while you continue working. When it finishes you are woken automatically with a \`<system-reminder>\` carrying the output — even if you have ended your turn and gone idle. If you have nothing else to do, END YOUR TURN: ending the turn is how you wait, and the wakeup finds you idle. Whether it has finished is never something you check. \`BashOutput\` is only for reading a running task's partial output when you need it to decide something you can't defer (it errors if the task has produced no output yet); \`KillShell\` stops a task. Backgrounding with \`&\` in the command does not make foreground Bash safe — always use \`run_in_background: true\` instead.
