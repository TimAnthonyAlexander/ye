...
export async function* runTurn(deps: TurnDeps): AsyncGenerator<Event, StopReason> {
  ...
  const tools = assembleToolPool({ mode: state.mode, rules: [...config.permissions?.rules ?? [], ...state.sessionRules] });
  ...
  for (const call of toolCalls) {
    ...
    if (decision.kind === "deny") {
      ...
      const startEvent: Event = { type: "tool.start", id: call.id, name: call.name, args: call.args };
